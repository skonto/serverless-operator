...
func (r *ReconcileKnativeKafka) transform(manifest *mf.Manifest, instance *serverlessoperatorv1alpha1.KnativeKafka) error {
	...
	tfs = append(append(tfs,
		...
		socommon.ConfigMapVolumeChecksumTransform(context.Background(), r.client, sets.NewString("config-tracing", "kafka-config-logging"))), socommon.DeprecatedAPIsTranformersFromConfig()...)
